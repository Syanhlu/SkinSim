#!/usr/bin/env python3
"""
Test Threads + Facebook + Polymarket simulation (5 rounds, all 3 platforms).
...
SIMULATION_REQUIREMENT = (
    "Simulate public reaction on Threads, Facebook, and Polymarket to this article "
...
def setup_simulation():
    ...
    os.makedirs(os.path.join(SIM_DIR, 'threads'), exist_ok=True)
    os.makedirs(os.path.join(SIM_DIR, 'facebook'), exist_ok=True)
    ...
    with open(os.path.join(SIM_DIR, 'facebook_profiles.json'), 'w') as f:
        json.dump(facebook_profiles, f, ensure_ascii=False, indent=2)
    print(f"  Facebook profiles: {len(facebook_profiles)}")

    # ── Threads profiles (CSV) ──
    ...
    threads_fields = ['user_id', 'name', 'username', 'user_char', 'description']
    threads_path = os.path.join(SIM_DIR, 'threads_profiles.csv')
    with open(threads_path, 'w', newline='', encoding='utf-8') as f:
    ...
    print(f"  Threads profiles: {len(agent_configs)}")
...
def run_simulation(config_path):
    ...
    banner("SIMULATION: 5 rounds, Threads + Facebook + Polymarket")
