"""
Wonderwall TikTok simulation preset script
...
Usage:
    python run_tiktok_simulation.py --config /path/to/simulation_config.json
    python run_tiktok_simulation.py --config /path/to/simulation_config.json --no-wait  # Close immediately after completion
"""
...
def setup_wonderwall_logging(log_dir: str):
    ...
    loggers_config = {
        ...
        "social.twitter": os.path.join(log_dir, "social.tiktok.log"),
...
try:
    ...
    from wonderwall.simulations.social_media import tiktok_simulation
...
class IPCHandler:
    ...
    def _get_interview_result(self, agent_id: int) -> Dict[str, Any]:
        ...
        db_path = os.path.join(self.simulation_dir, "tiktok_simulation.db")
...
class TikTokSimulationRunner:
    """TikTok simulation runner"""

    # Available TikTok actions (excluding INTERVIEW, which can only be
    # triggered manually via ManualAction). Sourced from
    # ActionType.get_default_tiktok_actions() so this stays in sync with
    # wonderwall.simulations.social_media.tiktok_simulation.default_actions
    # (no QUOTE_POST — duet/stitch is a stretch goal, not v1).
    ...
    def _get_profile_path(self) -> str:
        ...
        return os.path.join(self.simulation_dir, "tiktok_profiles.json")

    def _get_db_path(self) -> str:
        """Get database path"""
        return os.path.join(self.simulation_dir, "tiktok_simulation.db")
    ...
    async def run(self, max_rounds: int = None):
        ...
        self.agent_graph = await generate_tiktok_agent_graph(
            profile_path=profile_path,
            model=model,
            available_actions=self.AVAILABLE_ACTIONS,
            simulation=tiktok_simulation,
        )
...
async def main():
    parser = argparse.ArgumentParser(description='Wonderwall TikTok Simulation')
    parser.add_argument(
        '--config',
        type=str,
        required=True,
