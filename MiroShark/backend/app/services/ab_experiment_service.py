"""
A/B experiment orchestration service.

Shared home for the logic that used to live entirely in
``backend/scripts/ab_experiment.py``: one prepared parent simulation is
created (or reused), one counterfactual branch is created per
variant x replicate, branch runs are started with a bounded active-run
count and polled until terminal, then metrics are collected and
small-sample statistics computed.

Two consumers:

* ``scripts/ab_experiment.py`` — the CLI, now a thin argparse wrapper
  around the module-level functions here (flags unchanged, ``--resume``
  and ``--dry-run`` intact).
* :class:`ABExperimentService` — the HTTP-facing async job runner behind
  ``/api/experiments/*``. Each experiment runs in a **background thread**
  (mirroring the thread-per-job pattern of
  :mod:`app.services.simulation_runner`), tracked in an in-process
  registry keyed by ``experiment_id``. The single-worker constraint is
  deliberate: one experiment at a time per backend instance — a second
  ``create_experiment`` while one is active raises
  :class:`ExperimentConflictError` (HTTP 409 at the blueprint).

State is persisted to ``backend/uploads/experiments/<experiment_id>/state.json``
after every transition — the exact schema the CLI already writes, plus
``experiment_id`` / ``hypothesis`` / ``status`` / ``progress`` / ``error`` —
so status polling reads disk, a backend restart doesn't lose finished
work, and the CLI's ``--resume`` mental model carries over unchanged.

The orchestration talks to the backend over internal HTTP exactly like
the CLI does (same endpoints, same internal-key header) — stdlib
``urllib`` only, no new dependencies.

Progress lines from the orchestration loop use ``print()`` on purpose:
the CLI's console UX depends on them, and under Flask they land in the
server console next to the simulation subprocess chatter. This module
is never imported by ``mcp_server.py``, so the stdout-is-for-MCP rule
is not in play.
"""
...
def save_state(out_dir, state):
    """Persist ``state.json`` — the single write path for every transition.

    Service-managed experiments (recognized by the ``status`` key the CLI
    never writes) get their ``progress`` block refreshed on every save so
    disk is always poll-ready. CLI-authored states keep the exact schema
    the harness has always written.
    """
    state["updated_at_utc"] = utc_now()
    if "status" in state:
        state["progress"] = compute_progress(state)
    write_json(pathlib.Path(out_dir) / STATE_FILE, state)
...
# ──────────────────────────────────────────────────────────────────────────
# Internal HTTP client (stdlib only)
# ──────────────────────────────────────────────────────────────────────────
...
def build_initial_state(
    *,
    ...
    """The CLI's state.json schema, shared verbatim between CLI and service."""
    return {
        "schema_version": 1,
        ...
        "parent": {
            ...
            "project_id": None,
            "graph_id": None,
            "phase": "provided" if parent_simulation_id else "not_created",
            "ready": False,
            "scenario_text": scenario_text,
        },
        "variants": variants,
        "runs": make_planned_runs(variants, replicates, trigger_round),
        "artifacts": {},
    }
...
def fetch_metrics(client, state, out_dir):
    for run in state.get("runs") or []:
        ...
        # Prediction-market cross-check for the results surface (best-effort;
        # sims without a Polymarket platform simply return an empty list).
        markets = client.request_json(
            "GET", "/api/simulation/" + sim_id + "/polymarket/markets", timeout=60
        )
        ...
        metrics["cost"] = cost
        metrics["polymarket_markets"] = markets
...
# ──────────────────────────────────────────────────────────────────────────
# Statistics (small-sample, replicate-level)
# ──────────────────────────────────────────────────────────────────────────
...
def compute_demographic_winners(state):
    ...
    for key in sorted(collector.keys()):
        ...
        if not means:
            continue
        # A segment where every variant scored 0.00% is a "tie" between
        # nothing and nothing — noise in the report, not a winner table row.
...
# ──────────────────────────────────────────────────────────────────────────
# Report rendering + artifact output
# ──────────────────────────────────────────────────────────────────────────
...
def endpoints_used():
    return [
        ...
        "GET /api/simulation/<id>/polymarket/markets",
    ]
...
# ──────────────────────────────────────────────────────────────────────────
# Service-side derivations: progress, agent counts, results contract
# ──────────────────────────────────────────────────────────────────────────


def compute_progress(state):
    """Poll-friendly progress block derived from the run list.

    ``current_round_max`` / ``rounds_per_run`` come from the freshest
    ``last_status`` snapshot (batch-status or run-status payload) any run
    has recorded — both carry ``current_round`` / ``total_rounds``.
    """
    runs = state.get("runs") or []
    runs_done = 0
    runs_active = 0
    current_round_max = None
    rounds_per_run = None
    for run in runs:
        status = str(run.get("status") or "").lower()
        phase = run.get("phase")
        if status in TERMINAL_ANY or phase in ("terminal", "metrics_fetched", "failed"):
            runs_done += 1
        elif phase in ("started", "running"):
            runs_active += 1
        last = run.get("last_status") or {}
        cur = last.get("current_round")
        tot = last.get("total_rounds")
        if isinstance(cur, (int, float)):
            current_round_max = max(current_round_max or 0, int(cur))
        if isinstance(tot, (int, float)) and tot:
            rounds_per_run = int(tot)
    return {
        "runs_total": len(runs),
        "runs_done": runs_done,
        "runs_active": runs_active,
        "current_round_max": current_round_max,
        "rounds_per_run": rounds_per_run,
    }


def run_agents_count(run):
    """Total judged agents for one replicate, from its demographics payload.

    Prefers ``agents_with_stance`` (the denominator the stance judge actually
    used) and falls back to ``total_agents``. Returns ``None`` when the
    payload is missing — callers must skip such runs rather than invent n.
    """
    demo = ((run.get("metrics") or {}).get("demographics") or {})
    data = demo.get("data") if isinstance(demo, dict) else None
    meta = data.get("meta") if isinstance(data, dict) else None
    if not isinstance(meta, dict):
        return None
    for key in ("agents_with_stance", "total_agents"):
        value = meta.get(key)
...
def run_amm_yes_probability(run):
    """Final AMM YES price for one replicate (best-effort, may be None)."""
    payload = ((run.get("metrics") or {}).get("polymarket_markets") or {})
    data = payload.get("data") if isinstance(payload, dict) else None
    markets = data.get("markets") if isinstance(data, dict) else None
    if isinstance(markets, list) and markets and isinstance(markets[0], dict):
        return as_float(markets[0].get("price_yes"))
    return None


def build_results_payload(experiment_id, state, computed):
    """Map experiment state → the TS ``ExperimentResults`` contract + ``raw``.

    Stance→binary mapping rule (the load-bearing decision): each agent's
    final stance from belief-drift is one Bernoulli observation.
    ``visitors`` = Σ agents across all completed replicates of a variant,
    ``conversions`` = Σ bullish agents (``round(bullish_pct/100 * agents)``
    per replicate — the closest integer to the count the percentage was
    derived from). This gives the two-proportion z-test a real n, while
    per-replicate means stay in ``raw`` / ``notes`` for honesty about
    clustering. Runs whose demographics payload carries no agent count are
    excluded from the pooled totals (never invent per-agent data) and noted.
    """
    variant_names = [v.get("name") for v in state.get("variants") or []]
    # Contract naming: first variant is "control", second is "treatment";
    # any additional variants keep their own names. Original names are
    # preserved in `raw.per_replicate` and `notes`.
    display_names = {}
    for idx, name in enumerate(variant_names):
        if idx == 0:
            display_names[name] = "control"
        elif idx == 1:
            display_names[name] = "treatment"
        else:
            display_names[name] = name

    totals = {name: {"visitors": 0, "conversions": 0} for name in variant_names}
    per_replicate = []
    replicate_pcts = {name: [] for name in variant_names}
    skipped = []
    ...
    for run in state.get("runs") or []:
        ...
        if str(run.get("status") or "").lower() not in TERMINAL_OK:
            continue
        bullish_pct = as_float(run.get("final_bullish_pct"))
        agents = run_agents_count(run)
        per_replicate.append(
            {
                ...
                "agents": agents,
                "amm_yes_probability": run_amm_yes_probability(run),
            }
        )
        if bullish_pct is None or agents is None:
            skipped.append(str(run.get("label") or variant))
            continue
        replicate_pcts[variant].append(bullish_pct)
        totals[variant]["visitors"] += agents
        totals[variant]["conversions"] += int(round(bullish_pct * agents / 100.0))

    notes = []
    for name in variant_names:
        pcts = replicate_pcts[name]
        rendered = ", ".join(fmt_num(p, 1) + "%" for p in pcts)
        notes.append("replicate means " + str(name) + ": [" + rendered + "]")
    if skipped:
        notes.append(
            "excluded from pooled counts (no agent count or stance): " + ", ".join(skipped)
        )
    ...
    return {
        ...
        "primaryUnit": "agents",
        "alpha": 0.05,
        "requiredSampleSizePerVariant": 0,
        "plannedDays": 1,
        "observedDays": 1,
        "variants": [
            {
                "name": display_names[name],
                "visitors": totals[name]["visitors"],
                "conversions": totals[name]["conversions"],
            }
            for name in variant_names
        ],
        "guardrails": [],
        ...
        "raw": {
            ...
            "per_demographic": (computed or {}).get("demographic_winners") or [],
            "welch_cli": (computed or {}).get("pairwise_vs_baseline") or {},
        },
    }


# ──────────────────────────────────────────────────────────────────────────
# The async job service
# ──────────────────────────────────────────────────────────────────────────


class ExperimentConflictError(RuntimeError):
    """Another experiment is actively running (single-worker constraint)."""


class ExperimentNotCompleteError(RuntimeError):
    """Results were requested before the experiment reached ``complete``."""
...
class ABExperimentService:
    """Background-thread A/B experiment runner with an on-disk state store.

    Mirrors :class:`SimulationRunner`'s shape: classmethod surface, an
    in-process registry, and a per-experiment directory under
    ``backend/uploads/experiments/`` holding ``state.json`` (poll target),
    ``results.json`` and ``report.md`` (written on completion).

    **Single worker.** One experiment at a time per backend instance — the
    orchestration drives up to ``parallel`` simulations already, and the
    engine's cost/step budget makes concurrent experiments a footgun. A
    create while one is active raises :class:`ExperimentConflictError`.
    The registry is in-process only: after a restart nothing is "active"
    (finished work is still on disk; an interrupted experiment stays in
    its last persisted status and can be resumed via the CLI's
    ``--resume`` against the same directory).
    """
    ...
    # ── directory / id helpers ───────────────────────────────────────────

    @classmethod
    def _experiment_dir(cls, experiment_id: str) -> pathlib.Path:
        ...
        return pathlib.Path(cls.EXPERIMENTS_DIR) / experiment_id

    @classmethod
    def _load_state(cls, experiment_id: str) -> Optional[dict]:
        try:
            state_path = cls._experiment_dir(experiment_id) / STATE_FILE
        except ValueError:
            return None
    ...
    @classmethod
    def get_active_experiment_id(cls) -> Optional[str]:
        """The experiment whose worker thread is currently alive, if any."""
        with cls._lock:
            if cls._active_id is None:
                return None
            thread = cls._threads.get(cls._active_id)
            if thread is not None and thread.is_alive():
                return cls._active_id
            # Thread finished (or died) without deregistering — clear it.
            cls._threads.pop(cls._active_id, None)
            cls._active_id = None
            return None
    ...
    def create_experiment(
        ...
        *,
        ...
        """Validate, persist the initial state, and launch the worker thread.

        Returns the 202 body: ``{experiment_id, parent_simulation_id, status}``.
        ``parent_simulation_id`` is ``None`` when the parent is created from
        scenario text — it appears in ``state.json`` (and the status payload)
        once the parent simulation exists.

        Raises :class:`ValueError` on invalid input and
        :class:`ExperimentConflictError` when another experiment is active.
        """
        if not scenario and not parent_simulation_id:
            raise ValueError("provide scenario or parent_simulation_id")
        if scenario and parent_simulation_id:
            raise ValueError("use scenario OR parent_simulation_id, not both")
        if not isinstance(variants, list) or len(variants) < 2:
            raise ValueError("provide at least 2 variants")
        names = []
        for variant in variants:
            if not isinstance(variant, dict):
                raise ValueError("each variant must be an object with name and text")
            name = str(variant.get("name") or "").strip()
            text = str(variant.get("text") or "").strip()
            if not name or not text:
                raise ValueError("each variant needs a non-empty name and text")
            if name in names:
                raise ValueError("duplicate variant name: " + name)
            names.append(name)
        if int(replicates) < 1:
            raise ValueError("replicates must be >= 1")
        if int(parallel) < 1:
            raise ValueError("parallel must be >= 1")
        if int(trigger_round) < 0:
            raise ValueError("trigger_round must be >= 0")
        if int(poll_interval) < 1:
            raise ValueError("poll_interval must be >= 1")

        if api_url is None:
            api_url = os.environ.get("MIROSHARK_API_URL") or (
                "http://127.0.0.1:"
                + os.environ.get("PORT", os.environ.get("FLASK_PORT", "5001"))
            )

        clean_variants = [
            {"name": str(v.get("name")).strip(), "text": str(v.get("text")).strip()}
            for v in variants
        ]

        with cls._lock:
            if cls._active_id is not None:
                thread = cls._threads.get(cls._active_id)
                if thread is not None and thread.is_alive():
                    raise ExperimentConflictError(
                        "experiment " + cls._active_id + " is already running "
                        "(single worker per instance)"
                    )
                cls._threads.pop(cls._active_id, None)
                cls._active_id = None

            experiment_id = "exp_" + uuid.uuid4().hex[:8]
            out_dir = cls._experiment_dir(experiment_id)
            ...
            state["hypothesis"] = str(hypothesis or "")
            state["status"] = "preparing"
            state["error"] = None
            save_state(out_dir, state)

            thread = threading.Thread(
                target=cls._run_experiment,
                args=(experiment_id,),
                name="ab-experiment-" + experiment_id,
                daemon=True,
            )
            cls._threads[experiment_id] = thread
            cls._active_id = experiment_id
    ...
    @classmethod
    def _run_experiment(cls, experiment_id: str) -> None:
        """Worker body: parent prep → branch runs → metrics → artifacts.

        Every phase mutation goes through :func:`save_state`, so status and
        progress land on disk after each transition. The internal key is
        read from the environment at run time — it is never persisted into
        ``state.json``.
        """
        out_dir = cls._experiment_dir(experiment_id)
        state = cls._load_state(experiment_id)
        if state is None:
            with cls._lock:
                cls._threads.pop(experiment_id, None)
                if cls._active_id == experiment_id:
                    cls._active_id = None
            return

        client = ApiClient(
            state.get("api_url"),
            os.environ.get("MIROSHARK_INTERNAL_KEY", ""),
        )
        poll_interval = int((state.get("config") or {}).get("poll_interval") or 30)
        parallel = int((state.get("config") or {}).get("parallel") or 2)

        try:
            state["status"] = "preparing"
            save_state(out_dir, state)

            create_parent(client, state, out_dir, poll_interval)

            state["status"] = "running"
            save_state(out_dir, state)

            run_branches(client, state, out_dir, parallel, poll_interval)
            fetch_metrics(client, state, out_dir)
            write_outputs(state, out_dir)

            completed = [
                run for run in state.get("runs") or []
                if str(run.get("status") or "").lower() in TERMINAL_OK
            ]
            if completed:
                state["status"] = "complete"
            else:
                state["status"] = "failed"
                state["error"] = state.get("error") or "no simulation run completed"
            save_state(out_dir, state)
        except Exception as exc:
            state["status"] = "failed"
            state["error"] = str(exc)
    ...
    # ── read surface (all disk-backed) ───────────────────────────────────

    @classmethod
    def get_status(cls, experiment_id: str) -> Optional[Dict[str, Any]]:
        """Status payload for polling, or ``None`` for an unknown id."""
        state = cls._load_state(experiment_id)
        if state is None:
            return None
        runs = []
        for run in state.get("runs") or []:
            last = run.get("last_status") or {}
            rounds_done = last.get("current_round")
            runs.append(
                {
                    "variant": run.get("variant"),
                    "replicate": run.get("replicate"),
                    "simulation_id": run.get("simulation_id"),
                    "state": run.get("terminal_status") or run.get("status") or run.get("phase"),
                    "rounds_done": rounds_done if isinstance(rounds_done, (int, float)) else None,
                }
            )
        return {
            "experiment_id": experiment_id,
            "status": state.get("status"),
            "progress": state.get("progress") or compute_progress(state),
            "runs": runs,
            "parent_simulation_id": (state.get("parent") or {}).get("simulation_id"),
            "error": state.get("error"),
        }

    @classmethod
    def get_results(cls, experiment_id: str) -> Optional[Dict[str, Any]]:
        """The TS ``ExperimentResults`` payload, or ``None`` for unknown id.

        Raises :class:`ExperimentNotCompleteError` while the experiment is
        still preparing/running or has failed.
        """
        state = cls._load_state(experiment_id)
        if state is None:
            return None
        if state.get("status") != "complete":
            raise ExperimentNotCompleteError(
                "experiment " + experiment_id + " is "
                + str(state.get("status") or "unknown") + ", not complete"
            )
        computed = None
        results_path = cls._experiment_dir(experiment_id) / RESULTS_FILE
        if results_path.exists():
            try:
                computed = (load_json(results_path) or {}).get("computed")
            except Exception:
                computed = None
        if computed is None:
            computed = compute_results(state)
        return build_results_payload(experiment_id, state, computed)

    @classmethod
    def list_experiments(cls) -> List[Dict[str, Any]]:
        """Every experiment on disk, newest first."""
        root = pathlib.Path(cls.EXPERIMENTS_DIR)
        if not root.exists():
            return []
        out = []
        for child in root.iterdir():
            ...
            if not isinstance(state, dict) or "status" not in state:
                continue  # not a service-managed experiment
            out.append(
                {
                    "experiment_id": state.get("experiment_id") or child.name,
                    "status": state.get("status"),
